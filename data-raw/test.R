...
x <- list(
  ...
  lins = dplyr::tibble(
    id = 1:8,
    e = 1:8,
    v1 = c(1, 2, 2, 7, 3, 4, 4, 7),
    v2 = c(2, 4, 7, 3, 4, 5, 6, 8)
  ),
  adjacency = matrix(0, 8, 8),
  incidence = matrix(0, 8, 8),
  d = rep(0, 8),
  q = 2,
  W = 8,
  M = 8
...
for (m in 1:x$M) {
  ...
  cs <- cumsum(dplyr::filter(x$lins, e == m) %>% dplyr::pull(length))
  x$lins$frac1[which(x$lins$e == m)] <- c(0, cs[-length(cs)]/cs[length(cs)])
  x$lins$frac2[which(x$lins$e == m)] <- dat$length/cs
}
x$incidence <- getIncidence(x)

small_gn2 <- x
class(small_gn2) <- "gn"
usethis::use_data(small_gn2, overwrite = TRUE)
